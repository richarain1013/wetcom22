...
1. 先退出已打开的企业微信  
2. 打开 **WeCom Launcher**  
3. 确认路径已定位到 `WXWork.exe`  
4. 批量数量设为 `8` 或 `10`，间隔保持默认  
5. 勾选「Windows：优先注册表 multi_instances」  
6. 点 **分批启动**，每个窗口扫码登录不同账号  

### 安全模式（本地用户隔离，推荐）

详见 [SAFE_MODE.md](./SAFE_MODE.md)。摘要：

1. **管理员**运行启动器  
2. 勾选「Windows 安全模式」→ 填密码 → **准备本地用户**  
3. 再 **分批启动**  

若第 2 个起起不来：

- **右键以管理员身份运行** 启动器再试  
- 看日志里是否提示互斥体释放失败 / 用户不存在  
